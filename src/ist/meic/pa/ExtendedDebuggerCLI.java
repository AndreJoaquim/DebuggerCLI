...
public class ExtendedDebuggerCLI {
	...
	public static Object initCommandLine(String invocationTargetClassName,
		...
		try {
			...
			try {
				...
				invocationTargetReturn = methodToInvoke.invoke(
						invocationTarget, invocationTargetMethodParams);
			...
			while (true) {
				...
				if (command.equals("Abort")) {
					...
					Class<?> invocationTargetClassToPrint = Class
							.forName(invocationTargetClassName);

					Field[] declaredFields = invocationTargetClassToPrint
							.getDeclaredFields();
				...
				} else if (command.equals("Return-Complex")) {
					...
					//if command has less than 2 arguments something its wrong
					if (split_input.length < 2) {
						System.out.println("Invalid command syntax");
						System.out.println("Usage: Return-Complex <class name> <field 1> <field 2> ... <field n>. For more info, use Help command");
						continue;
					}

					String className = split_input[1];
					...
					try {
						...
						System.out.println("The class of the return instance can not be found. Try another command");
						continue;
					...
					System.out
							.println("Return-Complex <class name> <field 1> <field 2> ... <field n>: Ignores the exception and continues the execution of the application assuming that the"
									+ "current method call returned a non-primitive instance of class <class name> and with a constructor that has fields <field 1> <field 2> ... <field n>" 
									+". For calls to methods returning void the <value> is ignored.");
					System.out
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (args.length < 1) {
				System.out
						.println("Usage: java ist.meic.pa.ExtendedDebuggerCLI <programToDebug> [<args>]");
				System.exit(1);
			}

			// DebuggerCLI Translator
			Translator translator = new ExtendedDCLITranslator();
